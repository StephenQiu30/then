...
草案，2026-08-09。关联 [`../prd/03-最小可行产品需求.md`](../prd/03-最小可行产品需求.md)、[`03-系统总体设计.md`](03-系统总体设计.md) 与 [`01-技术选型.md`](01-技术选型.md)。本文定义逻辑模型；评审通过前不向 `backend/schema.sql` 写入业务 DDL。
...
不可变字段包括加密名称与地址、经纬度、坐标系、来源（`manual | mapkit`）、水平精度和创建时间。当前不保存第三方地点 ID。常用地点可指向最新快照，但历史计划和行程继续引用当时快照。
...
每台设备最多存在一个 active Journey。异常退出后保留恢复标记，下次启动让用户继续、完成或丢弃，不自动推断到达。
