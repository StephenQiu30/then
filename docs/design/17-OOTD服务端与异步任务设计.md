...
当前产品需求事实源是 [OOTD 产品需求](../prd/10-OOTD产品需求.md)。

本设计与以下已批准设计共同组成 OOTD 设计集：

- [OOTD 产品总体设计](10-OOTD产品总体设计.md)
- [数字形象与照片采集设计](11-数字形象与照片采集设计.md)
- [数字衣橱与衣物录入设计](12-数字衣橱与衣物录入设计.md)
- [穿搭推荐设计](13-穿搭推荐设计.md)
- [AI 虚拟试穿设计](14-AI虚拟试穿设计.md)
- [动态预览设计](15-动态预览设计.md)
- [穿搭记录与反馈设计](16-穿搭记录与反馈设计.md)
- [OOTD 权限隐私与安全设计](18-OOTD权限隐私与安全设计.md)
...
7. 静态试穿失败不影响结构化推荐；动态预览失败不影响静态结果，边界分别见 [AI 虚拟试穿设计](14-AI虚拟试穿设计.md) 和 [动态预览设计](15-动态预览设计.md)。
...
| `outfit` / `outfit_item` | 场景、组成单品、推荐/实际来源、版本 | 推荐与实际穿着不可混写；反馈见 16 号设计 |
...
| 9 | 用户级限流与配额预检 | 作用域为已认证用户和 operation；最终计费/配额仍在 PostgreSQL 结算 |
| 10 | OpenAPI 请求校验 | 校验 path/query/header/body；未知字段策略由契约固定 |
| 11 | 幂等获取/回放 | 用用户 + operation + key 和 request hash 判定；相同请求回放，不同请求返回 409 |
| 12 | 资源授权 | Service/Repository 再校验 `user_id` 与资源归属，不接受客户端声明的所有者 |
...
- C 端首版以 Sign in with Apple 作为主账号入口；若实施“先体验后登录”，只允许设备绑定的匿名云主体，并提供无损升级到 Apple 账号的合并流程。匿名云账号未进入当前实施阶段时，未登录用户保持纯本地，并在首次云生成或同步前登录。
...
本节是服务端最低要求，更完整边界见 [OOTD 权限隐私与安全设计](18-OOTD权限隐私与安全设计.md)。
...
- [ ] 10 号 PRD、当前实施计划、验收和隐私/供应商准入记录一致且均在有效期内。
