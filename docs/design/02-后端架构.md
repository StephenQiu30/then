...
完整版本与 schema 工具约束见 [`01-技术选型.md`](01-技术选型.md)。
...
- P1 首版服务端只使用 Atlas Community 可声明管理的 enum、table、column、constraint、index 和 comment；Repository 显式写入时间戳与 Outbox，不依赖 trigger。
...
- extension、function、trigger、RLS、seed DML、回填或大表在线变更不在当前 Community 声明式基线内；需要这些能力时必须重新批准工具版本或版本化迁移方案。
